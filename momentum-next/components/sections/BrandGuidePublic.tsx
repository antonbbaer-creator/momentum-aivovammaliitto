...
import { AVL_LOGO_VARIANTS, AVL_FONTS, AVL_TEMPLATES, AVL_GUIDE_PDF, AVL_GUIDE_TOTAL_PAGES } from '@/lib/avl-brand-assets';
          ...
          {AVL_LOGO_VARIANTS.map(v => (
            <div key={v.id} style={{ border: '1px solid var(--border)', borderRadius: 'var(--rl)', padding: '0.85rem', background: 'var(--paper-l)', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', marginTop: 'auto' }}>
                <a className="btn btn-primary" style={{ fontSize: '0.78rem', padding: '0.4rem 0.7rem', textAlign: 'center' }} href={v.pngTransparent} target="_blank" rel="noopener noreferrer" download>Läpinäkyvä tausta (PNG)</a>
                <a className="btn btn-secondary" style={{ fontSize: '0.78rem', padding: '0.4rem 0.7rem', textAlign: 'center' }} href={v.pngWhite} target="_blank" rel="noopener noreferrer" download>Valkoinen tausta (PNG)</a>
              </div>
            </div>
          ))}
        </div>
...
function AineistotCard() {
  if (AVL_TEMPLATES.length === 0) return null;
  return (
    <div style={card}>
