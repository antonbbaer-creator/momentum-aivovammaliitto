...
export default function PublicationsSection() {
      ...
      <>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '1.5rem' }}>
          ...
          <div>
            ...
            {canEdit && <button className="btn btn-sm" onClick={() => deletePub(detail.id)} style={{ width: '100%', color: 'var(--red)', border: '1px solid rgba(239,107,107,.3)', background: 'rgba(239,107,107,.05)' }}>Poista julkaisu</button>}
          </div>
        </div>
      </>
    );
  ...
  return (
    <>
      <div style={{ display: 'flex', background: 'var(--elev)', borderRadius: 'var(--r)', padding: '3px', marginBottom: '1rem', width: 'fit-content', flexWrap: 'wrap' }}>
        {commsCategories.map(c => (
          <button key={c.id} className={`cal-view-btn ${categoryFilter === c.id ? 'act' : ''}`} onClick={() => setCategoryFilter(c.id)}>
            {c.label} {c.id !== 'all' && <span style={{ opacity: .5, marginLeft: '.25rem' }}>({pubs.filter(p => (p.category || 'some') === c.id).length})</span>}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '.5rem' }}>
        <div style={{ display: 'flex', gap: '.3rem', flexWrap: 'wrap' }}>
          {[{ k: 'all', l: 'Kaikki' }, { k: 'draft', l: 'Luonnokset' }, { k: 'ready', l: 'Valmiit' }, { k: 'published', l: 'Julkaistut' }].map(f => (
            <button key={f.k} className="btn btn-ghost btn-sm" onClick={() => setStatusFilter(f.k)}
