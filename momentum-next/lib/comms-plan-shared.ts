...
export function normalizeCommsPlan(p: any): CommsPlan {
  ...
  return {
    ...
    festivalDates: p.festivalDates || '',
    summary: p.summary || '',
    mission: p.mission || '',
    visitorGoal: typeof p.visitorGoal === 'number' ? p.visitorGoal : 0,
    visitorBaseline: typeof p.visitorBaseline === 'number' ? p.visitorBaseline : 0,
    volunteerGoal: typeof p.volunteerGoal === 'number' ? p.volunteerGoal : 0,
    volunteerBaseline: typeof p.volunteerBaseline === 'number' ? p.volunteerBaseline : 0,
    responsibleMemberId: p.responsibleMemberId || '',
    responsibleTeamId: p.responsibleTeamId || '',
    activeFrom: p.activeFrom || '',
    visualIdentityDeadline: p.visualIdentityDeadline || '',
    kickoffNote: p.kickoffNote || '',
    strategicMoves:  Array.isArray(p.strategicMoves)  ? p.strategicMoves  : [],
    kpis:            Array.isArray(p.kpis)            ? p.kpis            : [],
    audienceMix:     Array.isArray(p.audienceMix)     ? p.audienceMix     : [],
    brandPillars:    Array.isArray(p.brandPillars)    ? p.brandPillars    : [],
    milestones:      Array.isArray(p.milestones)      ? p.milestones      : [],
    monthTargets:    Array.isArray(p.monthTargets)    ? p.monthTargets    : [],
    phases:          Array.isArray(p.phases)          ? p.phases          : [],
    campaigns:       Array.isArray(p.campaigns)       ? p.campaigns       : [],
    channelMatrix:   Array.isArray(p.channelMatrix)   ? p.channelMatrix   : [],
